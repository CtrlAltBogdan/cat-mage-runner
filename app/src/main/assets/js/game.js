...
function updatePlayerPosition() {
    ...
    if (playerSettings.movingRight && playerSettings.x < gameContainer.offsetWidth - player.offsetWidth) {
        playerSettings.x += playerSettings.speed;
    }
...
function playDeathAnimation() {
    ...
    function animateExplosion() {
        ...
        if (currentFrame > 0) {
            const previousFrameImage = gameContainer.querySelector(`img[src="${deathAnimationFrames[currentFrame - 1]}"]`);
...
function increaseDifficulty() {
    ...
    gameSettings.obstacleCreationChance = Math.min(0.5, gameSettings.obstacleCreationChance + 0.05);
    gameSettings.itemCreationChance = Math.min(0.5, gameSettings.itemCreationChance + 0.05);

    gameSettings.spawnDelayRange.max = Math.max(500, gameSettings.spawnDelayRange.max - 100);
}
...
function spawnObstacles() {
    ...
    setTimeout(spawnObstacles, getRandomDelay(gameSettings.spawnDelayRange.min, gameSettings.spawnDelayRange.max));
}
...
function spawnItems() {
    ...
    setTimeout(spawnItems, getRandomDelay(gameSettings.spawnDelayRange.min, gameSettings.spawnDelayRange.max));
}
...
playAgainButton.addEventListener("touchstart", () => {  // Добавлен обработчик touchstart
    gameOverScreen.style.display = "none";
    resetGame();
});
...
player.addEventListener("touchmove", (e) => {
    if (playerSettings.isTouchActive) {
        ...
        playerSettings.x = Math.max(0, Math.min(playerSettings.x, gameContainer.offsetWidth - player.offsetWidth));
        player.style.left = `${playerSettings.x}px`;
